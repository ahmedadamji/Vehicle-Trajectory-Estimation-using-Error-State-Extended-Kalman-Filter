...
var_lidar = 1.00
# Part 2
# The calibration between the IMU and the LIDAR is incorrect.
# Therefore increasing the variance of the LIDAR will make the filter trust the LIDAR less.
# var_imu_f = 0.10
# var_imu_w = 0.001
# var_gnss  = 0.01
# var_lidar = 80
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    # Inputs:
    #   sensor_var: variance for the sensor measurement
    #   p_cov_check:  covariance of the state coming into the measurement update
    #   y_k:  measured position
    #   p_check:  checked position
    #   v_check:  checked velocity
    #   q_check:  checked orientation
    ...
    # p_check is the checked position
    # The dimensions of K are 9x3
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    F_k[3:6, 6:9] = -C_ns @ skew_symmetric(imu_f.data[k-1].reshape((3, 1)))
    Q_k = np.identity(6)
    Q_k[0:3, 0:3] = var_imu_f * delta_t ** 2 * np.identity(3)
    Q_k[3:6, 3:6] = var_imu_w * delta_t ** 2 * np.identity(3)
